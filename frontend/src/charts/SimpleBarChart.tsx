...
function SimpleBarChart({ data, categoryKey, valueKey, valueLabel, highlightIndex }: SimpleBarChartProps) {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 4, right: 12, left: -8, bottom: 0 }}>
        ...
        />
        <Tooltip content={<CustomTooltip valueLabel={valueLabel} />} cursor={{ fill: "var(--gridline)" }} />
        <Bar dataKey={valueKey} radius={[4, 4, 0, 0]} maxBarSize={56} isAnimationActive={false}>
          {data.map((_, i) => (
            <Cell
              key={i}
              fill={i === highlightIndex ? "var(--status-critical)" : "var(--series-1)"}
            />
          ))}
        </Bar>
      </BarChart>
